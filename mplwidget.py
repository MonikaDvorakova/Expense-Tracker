...
# The central widget of the graphical output window.
# The class of the widget is set in qt designer.
class MplWidget(QtWidgets.QWidget):
    ...
    def pandasConvert(self, data, index):
        """Convert data into pandas data frame. Groups data by
        the selected index. Used for the bar charts.
        """
        if data:
            items = pandas.DataFrame(data,
                                     columns=['cathegory', 'person', 'date',
                                              'amount', 'id'])
    ...
    def pandasConvertDateChart(self, data, index):
        """Converts data into pandas dataframe. The data are used for
        the chart depicting expenses for the cathegories in time.
        Resulting chart depicts increase of the expenses for particular
        cathegories in week intervals.
        """
        if data:
                    ...
                    for _, row in items_sorted.iterrows():
                        ...
                        if cathegoryInData == cathegory:
                            if week < date <= dates[nextDate]:
                                totalSum += amount
                    try:
                        (ExpensesInTimeForCathegories[cathegory].
                         append(totalSum))
                    except IndexError:
                        pass
                totalSum = 0
    ...
    def listOfDates(self, data):
        """Return list of dates used in the time chart.
        The dates have week interval.
        """
        listOfWeeks = []
    ...
    def plotCharts(self):
        """Plot charts."""
        self.pandasDataPerson.plot.barh(y='amount', ax=self.canvas.axes2,
                                        fontsize=8, title='Purchases for '
                                        'particular persons', legend=False)
        self.canvas.axes2.set_xlabel('amount (kč)', fontsize=10)
        self.canvas.axes2.set_ylabel('')
        self.pandasDataCathegory.plot.barh(y='amount', ax=self.canvas.axes1,
                                           fontsize=8, title='Purchases for '
                                           'particular cathegories',
                                           legend=False)
        self.canvas.axes1.set_xlabel('amount (kč)', fontsize=10)
        self.canvas.axes1.set_ylabel('')
        self.pandasDataAmountInTime.plot.area(ax=self.canvas.axes3,
                                              fontsize=8, title='Time graph of'
                                              ' expences for particular '
                                              'cathegories')
