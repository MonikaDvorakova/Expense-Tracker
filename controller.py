...
class TrackerCtrl():
    def __init__(self, view):
        ...
        self.queryCreateTablePurchase = """
                                        ...
                                        (id INTEGER PRIMARY KEY AUTOINCREMENT,
                                         ...
                                         id_person INTEGER,
                                         FOREIGN KEY(id_cathegory)
                                         REFERENCES cathegory(id)
                                         FOREIGN KEY(id_person)
                                         REFERENCES person(id))
                                        """
        self.queryModel = """SELECT C.name, Pe.name, P.date, P.amount, P.id
                             FROM purchase P
        ...
        self.header.setSectionResizeMode(QtWidgets.QHeaderView.
                                         ResizeMode.Stretch)  # Columns are stretched according to the widget width
        ...
        self._view.dateFilterFrom.setDate(self.setFilterDateFrom
                                          (self._model._data))
        ...
        self.queryRemoveCathegory = """DELETE FROM cathegory
                                       WHERE name = (?)"""
        ...
        self.fillDropBoxFilter(self.connection, 'cathegory',
                               self._view.boxCathegoryFilter)
        self.fillDropBoxFilter(self.connection, 'person',
                               self._view.boxPersonFilter)
    ...
    def setFilterDateFrom(self, data):
        ...
        if data != self.emptyData:
            items = pandas.DataFrame(data, columns=['cathegory', 'person',
                                                    'date', 'amount', 'id'])
    ...
    def convertDateInData(self, data):
        """Change format of the date in the data.
        New format is used in the Tableview."""
        for index, row in enumerate(data):
            list_from_tuple = list(row)
            list_from_tuple[2] = (datetime.
                                  strptime(list_from_tuple[2],
    ...
    def _buttonClick(self):
        ...
        try:
            ...
            self.showMessageBox("Write the price of the purchase"
                                " as a whole number")
        else:
            ...
            if boxPerson and boxCathegory:
                idPerson = int(self.findIdOfItem(self.connection, queryPerson,
                                                 boxPerson)[0][0])
                idCathegory = int(self.findIdOfItem(self.connection,
                                                    queryCathegory,
                                                    boxCathegory)[0][0])
                query = """INSERT INTO purchase (amount, date, id_cathegory,
                                                 id_person) VALUES (?, ?, ?, ?)
                        """
    ...
    def btnFilterClick(self):
        ...
        param = None
        queryPerCat = """
                    SELECT C.name, Pe.name, P.date, P.amount, P.id
                    FROM purchase P
                    INNER JOIN person Pe ON P.id_person=Pe.id
                    INNER JOIN cathegory C ON P.id_cathegory=C.id
        ...
        queryPerson = """
                ...
                """
        queryDates = """
                    SELECT C.name, Pe.name, P.date as date, P.amount, P.id
                    FROM purchase P
                    INNER JOIN person Pe ON P.id_person=Pe.id
                    INNER JOIN cathegory C ON P.id_cathegory=C.id
                    WHERE p.date BETWEEN (?) AND (?)
        ...
        self.filterDateFrom = (self._view.dateFilterFrom.
                               date().toString('yyyy-MM-dd'))
        self.filterDateTo = (self._view.dateFilterTo.
                             date().toString('yyyy-MM-dd'))
    ...
    def dialogExecutionAdd(self, dialogWindow, queryDatabase,
                           table, box1, box2):
    ...
    def openDialog(self):
        """Depending on pressed button from toolbar, it opens dialog window,
        which serves for adding or removing items to or from the tables and
        for opening another project.
        """
        pressedBtn = self._view.sender().text()
        if pressedBtn == 'new cathegory':
            self.dialogExecutionAdd(self._view.dialogCathegory,
                                    self.queryAddCathegory, 'cathegory',
                                    self._view.boxCathegory,
                                    self._view.boxCathegoryFilter)
        elif pressedBtn == 'new person':
            self.dialogExecutionAdd(self._view.dialogPerson,
                                    self.queryAddPerson, 'person',
                                    self._view.boxPerson,
                                    self._view.boxPersonFilter)
    ...
    def sort(self):
        ...
        try:
            ...
            queryPC = ('SELECT C.name, Pe.name, P.date, P.amount, P.id '
                       'FROM purchase P '
                       'INNER JOIN person Pe ON P.id_person=Pe.id '
                       'INNER JOIN cathegory C ON P.id_cathegory=C.id '
                       'WHERE P.date BETWEEN (?) AND (?) '
                       ...
                       'AND c.name=(?) '
                       'ORDER BY {} {}'.format(sortedBy, sorting))
            queryCathegory = ('SELECT C.name, Pe.name, P.date, P.amount, P.id '
                              'FROM purchase P '
                              'INNER JOIN person Pe ON P.id_person=Pe.id '
                              'INNER JOIN cathegory C ON P.id_cathegory=C.id '
                              'WHERE P.date BETWEEN (?) AND (?) '
                              'AND c.name=(?) '
                              'ORDER BY {} {}'.format(sortedBy, sorting))
            queryPerson = ('SELECT C.name, Pe.name, P.date, P.amount, P.id '
                           'FROM purchase P '
                           'INNER JOIN person Pe ON P.id_person=Pe.id '
                           'INNER JOIN cathegory C ON P.id_cathegory=C.id '
                           'WHERE P.date BETWEEN (?) AND (?) '
                           'AND Pe.name=(?) '
                           'ORDER BY {} {}'.format(sortedBy, sorting))
            queryDates = ('SELECT C.name, Pe.name, P.date as date, P.amount, '
                          'P.id FROM purchase P '
                          'INNER JOIN person Pe ON P.id_person=Pe.id '
                          'INNER JOIN cathegory C ON P.id_cathegory=C.id '
                          'WHERE p.date BETWEEN (?) AND (?) '
                          'ORDER BY {} {}'.format(sortedBy, sorting))
            ...
            if not self.filterDateFrom and not self.filterDateTo:
                ...
                if self.filterCathegory == '-' and self.filterPerson == '-':
                    ...
                    query = queryPC
                    param = (self.filterDateFrom, self.filterDateTo,
                             self.filterPerson, self.filterCathegory)
                    self.ModelData(self.connection, query, param)

    def showGraphics(self):
        if len(self._model._data) > 1:
            self.MplWidget.pandasDataCathegory = (self.MplWidget.pandasConvert
                                                  (self._model._data,
                                                   'cathegory'))
            self.MplWidget.pandasDataPerson = (self.MplWidget.pandasConvert
                                               (self._model._data, 'person'))
            self.MplWidget.pandasDataAmountInTime = (self.MplWidget.
                                                     pandasConvertDateChart
                                                     (self._model._data,
                                                      'date'))
